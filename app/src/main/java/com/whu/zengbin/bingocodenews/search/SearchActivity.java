...
public class SearchActivity extends BaseActivity
  ...
  private void configSearchView() {
    ...
    mSearchAutoComplete.setHintTextColor(
        getResources().getColor(android.R.color.darker_gray));//设置提示文字颜色
    ...
    LinearLayout.LayoutParams lpsearchbt =
  ...
  private void loadResult(String content, boolean isLoadMore) {
    if (isLoadMore) {
      ...
      CodeNewsApp.getInstance().getmNewsImpl().getNewsSearchResultList("all", page, content,
            ...
            public void onResponse(ResponseBody response) {
              ...
              try {
                ...
                JSONArray resultArray =
                    responsejson.optJSONArray(ConstraintUtil.RESULTS);
                infolist.clear();
                for (int i = 0; i < resultArray.length(); i++) {
                  NewsInfo info = new NewsInfo(resultArray.getJSONObject(i));
                  LogUtil.i(TAG, "item " + i + info.getType());
